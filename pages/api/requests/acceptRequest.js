...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (request.expired || request.expireOn < new Date()) {
            return res.status(400).json({ success: false, msg: "Referral request is already expired" });
        }
        ...
        if (request.status === 'Cancled') {
            return res.status(400).json({ success: false, msg: "Referral request has already been canceled" });
        }
        ...
        if (emailInfo) {
            return res.status(200).json({ success: true, msg: 'Referral request accepted and email notification sent' });
        } else {
